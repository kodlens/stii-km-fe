...
const SubjectHeadingLabel = forwardRef<SubjectHeadingRef, SubjectHeadingProps>(({ search, subject, sh }, ref) => {

    
    const { data, isFetching, error, refetch } = useQuery({
        queryKey: ['subjectSubjectHeadings', subject],
        queryFn: async () => {
            const res = await axios.get(`${config.baseUri}/api/subject-headings/subject-headings?key=${search}&subj=${subject}&sh=${sh}`)
            return res.data
        },
